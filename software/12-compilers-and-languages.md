...
Compilers are where the abstractions of every language in this curriculum meet the silicon of [01-engineering-embedded-firmware.md](../engineering/01-embedded-firmware.md). The performance gains you chase in [13-software-performance-engineering.md](13-performance-engineering.md) are almost all *compiler* gains; the safety guarantees of [11-software-systems-programming-rust.md](11-systems-programming-rust.md) are *enforced* by a compiler's static analysis; the real-time determinism of [04-foundations-modern-cpp-realtime.md](../foundations/04-modern-cpp-realtime.md) depends on knowing what code the compiler generates. The first-principles habit of [01_first_principles_systems_engineering.md](../foundations/01-first_principles_systems_engineering.md) — refusing to treat any layer as unopenable — is exactly the mindset this module rewards. It pairs with [12-software-compilers-and-languages.md](12-compilers-and-languages.md)'s siblings across the band and with the career framing of [03-career-software-engineering.md](../career/03-software-engineering.md).
...
A syntactically valid program can still be nonsense: `"hello" * 3.0`, calling an undeclared function, using a moved value (the Rust borrow check of [11-software-systems-programming-rust.md](11-systems-programming-rust.md)). **Semantic analysis** walks the AST and checks meaning:
...
- **Ahead-of-Time (AOT)** compiles fully to native code before running. Predictable, no warm-up, no runtime — the only acceptable model for hard-real-time and embedded ([04-foundations-modern-cpp-realtime.md](../foundations/04-modern-cpp-realtime.md)).
...
- **Performance reasoning** — when you understand inlining and vectorization you write code the optimizer can actually accelerate, and you read the emitted assembly (via `godbolt.org`, the Compiler Explorer) to confirm it (the core method of [13-software-performance-engineering.md](13-performance-engineering.md)).
